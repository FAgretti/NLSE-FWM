...
"""C: cp.ndarray = NotImplemented
    ...
    def _step_impl(self):
        ...
        min_step = 10 * cp.abs(np.nextafter(t, self.direction * np.inf) - t)
...
def rk45_gpu(f, t0, y0, t_final, tol=1e-6, max_steps=10000, z_locs = None):
    ...
    a = cp.array([0, 1/5, 3/10, 4/5, 8/9, 1, 1])
    b = cp.array([
        [0, 0, 0, 0, 0, 0],
        [1/5, 0, 0, 0, 0, 0],
        [3/40, 9/40, 0, 0, 0, 0],
    ...
    c4 = cp.array([35/384, 0, 500/1113, 125/192, -2187/6784, 11/84, 0])
    c5 = cp.array([5179/57600, 0, 7571/16695, 393/640, -92097/339200, 187/2100, 1/40])
    ...
    for _ in range(max_steps):
        ...
        k = cp.zeros((7, len(y0)),dtype=complex)
        for i in range(7):
            ti = t + a[i] * h
            yi = y + h * cp.sum(b[i, :i, None] * k[:i], axis=0)
            k[i] = f(ti, yi)
        
        # Compute fourth and fifth order estimates
        y4 = y + h * cp.sum(c4[:, None] * k, axis=0)
        y5 = y + h * cp.sum(c5[:, None] * k, axis=0)
        
        # Error estimation
        err = cp.linalg.norm(y5 - y4, ord=2) / cp.linalg.norm(y5, ord=2)
        ...
        h = cp.clip(h_new, min_factor * h, max_factor * h)
...
def rk45_solver(fun, z_span, y0, args=(), atol=1e-6, rtol=1e-3, max_step=0.1, zlocs=None):
    z0, zf = z_span
    z = z0
    y = cp.array(y0, dtype=cp.complex128)
    h = max_step  # Paso inicial
    solz = []
    sol_z = cp.empty(zlocs)
    sol_y = cp.zeros((zlocs, len(y0)), dtype=cp.complex128)
    #sol_y = cp.array(soly.T)
    print("sol_y:", np.shape(sol_y))
    
    if zlocs is None:
        z_locs = cp.arange(z0, zf, max_step)
    else:
        z_locs = cp.linspace(z0, zf, zlocs+1)
    ...
    while z < zf:
        ...
        error = cp.linalg.norm(y_new - (y + 16*k1/135 + 6656*k3/12825 + 28561*k4/56430 - 9*k5/50 + 2*k6/55))
